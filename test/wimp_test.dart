...
void main() {
  group('WimpPlugin static methods', () {
    ...
    test('idToExtendedId returns correct prefix for MSArtistTracklist', () {
      final result = WimpPlugin.idToExtendedId(
        'tracklistid_12345',
        MSArtistTracklist,
      );
  ...
  group('WimpPlugin formUri', () {
    ...
    test('formUri for MSTrack without mime type', () {
      final content = {'item_id': 'trackid_12345', 'service_id': 20};
      final result = WimpPlugin.formUri(content, MSTrack);
      expect(result, equals('x-sonos-http:trackid_12345.?sid=20&flags=32'));
    });

    test('formUri for MSAlbum', () {
      final content = {'extended_id': '0004002calbumid_12345'};
    ...
    test('formUri for MSAlbumList', () {
      final content = {'extended_id': '000d006clistid_12345'};
    ...
    test('formUri for MSPlaylist', () {
      final content = {'extended_id': '0006006cplaylistid_12345'};
    ...
    test('formUri for MSArtistTracklist', () {
      final content = {'extended_id': '100f006ctracklistid_12345'};
  ...
  group('WimpPlugin search methods', () {
    ...
    test(
      'getMusicServiceInformation throws on invalid search type',
      () async {
    ...
    test(
      'getTracks makes correct HTTP request',
      () async {
        String? capturedBody;
        String? capturedUrl;
        ...
        expect(
          capturedUrl,
          equals('http://client.wimpmusic.com/sonos/services/Sonos'),
        );
        expect(
          capturedSoapAction,
          equals('"http://www.sonos.com/Services/1.1#search"'),
        );
    ...
    test(
      'getAlbums makes correct HTTP request',
      () async {
        String? capturedBody;
    ...
    test(
      'getArtists makes correct HTTP request',
      () async {
        String? capturedBody;
    ...
    test(
      'getPlaylists makes correct HTTP request',
      () async {
        String? capturedBody;
  ...
  group('WimpPlugin browse', () {
    test(
      'browse root makes correct HTTP request',
      () async {
        String? capturedBody;
        String? capturedSoapAction;
        ...
        expect(
          capturedSoapAction,
          equals('"http://www.sonos.com/Services/1.1#getMetadata"'),
        );
    ...
    test(
      'browse with valid item makes correct HTTP request',
      () async {
        String? capturedBody;
  ...
  group('WimpPlugin error handling', () {
    test(
      'search error throws SoCoUPnPException',
      () async {
        final mockClient = MockClient((request) async {
          return http.Response(
  ...
  group('WimpPlugin XML body generation', () {
    test(
      'search body contains correct SOAP headers',
      () async {
        String? capturedBody;
        final mockClient = MockClient((request) async {
          capturedBody = request.body;
        ...
        expect(
          capturedBody!.contains('<sessionId>test-session-123</sessionId>'),
          isTrue,
        );
        expect(
          capturedBody!.contains('<deviceId>XX-XX-XX-XX-XX-XX</deviceId>'),
          isTrue,
        );
        expect(
    ...
    test(
      'search body contains search term and type',
      () async {
        String? capturedBody;
        final mockClient = MockClient((request) async {
          capturedBody = request.body;
  ...
  group('WimpPlugin constants', () {
    ...
    test('exception codes are defined', () {
      const codes = {'unknown': 20000, 'ItemNotFound': 20001};
