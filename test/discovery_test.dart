...
void main() {
  ...
  group('discover', () {
    test(
      'throws ArgumentError for invalid interface address',
      () async {
        expect(
    ...
    test(
      'accepts valid IPv4 interface address format',
      () async {
    ...
    ); // Longer timeout for network discovery

    test(
      'returns null on timeout with no devices',
      () async {
  ...
  group('anySoco', () {
    test(
      'returns existing visible instance if available',
      () async {
    ...
    test(
      'calls discover when no visible instances found',
      () async {
  ...
  group('byName', () {
    test(
      'returns null when no devices found',
      () async {
  ...
  group('scanNetwork', () {
    test(
      'returns null when no Sonos devices found on network',
      () async {
        // Scan a small non-existent network range
        final result = await scanNetwork(
          networksToScan: [
            '10.255.255.0/30',
          ], // Tiny network, unlikely to have Sonos
          scanTimeout: 0.1,
          maxThreads: 2,
        );
    ...
    test(
      'handles invalid network CIDR notation',
      () async {
    ...
    test(
      'empty networksToScan list returns null',
      () async {
        final result = await scanNetwork(networksToScan: [], scanTimeout: 0.1);
    ...
    test(
      'uses auto-discovered networks when networksToScan is null',
      () async {
  ...
  group('scanNetworkByHouseholdId', () {
    test(
      'returns null when household not found',
      () async {
  ...
  group('socoClassFactory configuration', () {
    ...
    test(
      'custom factory is called during scanNetwork',
      () async {
    ...
    test(
      'factory path is used when factory is set',
      () async {
    ...
    test(
      'factory is used when device found via scanNetwork',
      () async {
  ...
  group('Discovery error handling and logging', () {
    ...
    test(
      'handles socket send errors gracefully',
      () async {
